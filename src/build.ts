...
export default async () => {
  ...
  modelsName.forEach((m) => {
    ...
    models.push(buffer.substring(0, buffer.indexOf("}")));
  });
  ...
  models.forEach((m) => {
    ...
    const props = m
      ...
      .map((v) => v.trim());
    props.forEach((p) => {
      if (p.includes("$unique") || p.includes("$id"))
        uniqueProps.push(p.split(" ")[0].toUpperCase());
    });
  ...
  models.forEach((m) => {
    ...
    const props = m
      ...
      .filter((v) => v.trim() != "")
      .map((v) => v.trim())
      .filter((p) => p.includes("$default"));
    props.forEach((p) => {
      let defaultValue: any = p.substring(
        p.indexOf("$default(") + 9,
        p.length - 1
      );
  ...
  models.forEach((m) => {
    ...
    const props = m
      ...
      .filter((v) => v.trim() != "")
      .map((v) => v.trim())
      .filter((p) => p.includes("$relation"));
    props.forEach((p) => {
      let relation = p.substring(p.indexOf("$relation(") + 10, p.length - 1);
      const field = relation
        .substring(relation.indexOf("fields:") + 7, relation.indexOf("]"))
        .replace("[", "")
        .trim()
        .toUpperCase();
      const references = relation
        .substring(
          relation.indexOf("references:") + 11,
          relation.indexOf("]", relation.indexOf("references:"))
        )
        .replace("[", "")
        .trim()
        .toUpperCase();
      relations[
        prefix +
          p.split(" ")[1].replace("*", "").replace("[]", "").toUpperCase()
      ] = {
        field,
        references,
      };
  ...
  for (let i = 0; i < models.length; i++) {
    ...
    props.forEach((p) => {
      ...
      if (type.includes("*")) {
        type = type.replace("*", "");
        imports += `import ${type.replace("[]", "")} from './${type.replace(
          "[]",
          ""
        )}Schema'\n`;
      }
      ...
      mType += `\n\t${property}: ${type}${
        props.indexOf(p) == props.length - 1 ? "\n" : ","
      }`;
    });
    typesEvals.push(
  ...
  for (let i = 0; i < models.length; i++) {
    ...
    clientTablesConstructor += `this.${name.toLowerCase()} = new AvantTable< ${name} >("${
      prefix + name.toUpperCase()
    }", ${JSON.stringify(modelsUniques[i])}, ${JSON.stringify(
      modelsDefaults[i]
    )}, ${JSON.stringify(modelsRelations[i])})\n\t\t`;
  }
