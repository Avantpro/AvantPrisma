...
class AvantTable<T> {
  ...
  public findMany(args?: findArgs<T>): string {
    ...
    const where = args.where
      ? this.#whereQuery(args.where, !!args.include)
      : "";
    const fields = args.select
      ? this.#fieldsQuery(args.select, !!args.include)
      : "*";
    ...
    const orderBy = args.orderBy
      ? this.#orderQuery(args.orderBy, !!args.include)
      : "";
  ...
  public findUnique(args: findUniqueArgs<T>): string {
    ...
    const first = args.take ? `FIRST ${args.take!}` : "";
    const skip = args.skip ? `SKIP ${args.skip!}` : "";
    if (!this.uniques.some((u) => whereCols.includes(u)))
      throw new Error("Find unique requires at least one unique property");
    return `SELECT ${first} ${skip} ${fields} FROM ${
      this.name
    } ${join} ${this.#whereQuery(args.where, !!args.include)}`;
  }
          ...
          switch (filter) {
            ...
            case "between":
              if (value[0] instanceof Date)
                value[0] = this.#dateToISO(value[0]);
              if (value[1] instanceof Date)
                value[1] = this.#dateToISO(value[1]);
    ...
    for (const key in include) {
      if (Object.prototype.hasOwnProperty.call(include, key)) {
        if (include[key]) {
          ...
          joinQuery =
            joinQuery +
            `LEFT JOIN ${k} ON ${this.name}.${this.relations[k].field} = ${k}.${this.relations[k].references} `;
        }
      }
    }
    return joinQuery;
